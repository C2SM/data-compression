...
4. The budget is capped by the node's memory. A node holds one shared copy of the sample, and each rank holds up to
   about twice the sample while it evaluates a pipeline (the decoded copy and the encoded bytes), plus the
   temporaries of about eight inner chunks:

   ```
   S + R × 2 × S + R × 8 × chunk  ≤  --memory-threshold × node memory      (S = sample, R = ranks per node)
   ```

   On a Santis node (849.6 GiB for the job, threshold 0.8, 16 MiB chunks) the sample is capped at 20.5 GiB with 16
   ranks per node, 10.4 GiB with 32 and 5.2 GiB with 64. A larger budget is shrunk (`[memcheck] auto-shrunk ...`).
...
The indices are taken among the time steps and levels on which the field varies (step 5), when it is constant on
some: a cloud field that is 0 on its top 40 levels gets its 3 levels from the other 80 (53, 80, 106), and a field
that varies on 2 levels only keeps those 2. The log line says what was kept:
...
one in memory per rank, a failed read retried once). The scan finds:

- the finite minimum and maximum. FixedScaleOffset's scale and offset are fitted to it: the filter does not clip,
  so never to the sample's range. `--phys-tolerance` is a fraction of it, and a field that already crosses
...
whose finite values are all one number, or that has none, needs no search: it is stored losslessly with Zstd.

Rank 0 then reads only the chosen slabs and puts them in shared memory, one copy per node. A sample with a single
value, or no finite value, while the field varies says nothing about the codecs: the field is skipped (`[var]
skipping ...`, an error when it was named with `--field-to-compress`); with the candidates of step 4 this takes a
field whose variation the chosen time steps and levels miss in every combination, and a larger
`--eval-data-size-limit` lets the sample reach it.

Rank 0 also computes, once, what the gates take from the whole sample: the q99 cut of `--extremes-sensitive`.
...
Only the lowest third of the levels (index 80 and above) lies below about 12 km. For a field that varies on every
level, the 3 × 3 minimum keeps one level there (100). With the default cascade policy, a 5 GiB budget keeps levels
84 and 108 (10 and 2 km) among its five, and 10 GiB keeps 82, 97 and 112 (11, 5 and 1.2 km) among its eight. Levels
on which a field is constant (clouds above their highest top) are not candidates (step 4), which moves the sample
of such fields down, but the levels it keeps are still spread by index.

To address: choose levels evenly in pressure (mass) or height rather than in index, or let a field entry restrict
the levels (to the troposphere, say).
...
Codecs that use the correlation between levels see levels about 40 apart in the sample and neighbours in
production: zfp encodes both chunk shapes as 2-D arrays of levels × cells (size-1 axes dropped), and its 4 × 4
blocks span 4 adjacent levels in production but 3 distant ones in the sample. On a 61-level `qv` field, zfp's
rank-wise mode (`zfpy`) compressed 1.3 to 2.7 times better with production chunks than with the sample's, while its
flattened mode (`zfpy_flat`) did not change: the sweep can prefer the flattened mode where production would favour
the rank-wise one, and the ratios it predicts for such codecs are pessimistic. Fields
