...
| `sweep_state_{var}.json` | What the recorded rows were measured on (dataset, sample shape, dtype and digest, full-field value range, sampling policy and vertical floor, chunk settings, the definitions of the error metrics, library versions and EBCC's env vars).  `--resume` reuses rows only while it matches; otherwise the field restarts from scratch. |
